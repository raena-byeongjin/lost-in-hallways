...
// 사육 리스트를 처리하기 위한 클래스
//-----------------------------------------------------------------------------------------------------------------------------------
public class CViewBreedList : ViewPanel
{
	//-------------------------------------------------------------------------------------------------------------------------------
	// -
	//-------------------------------------------------------------------------------------------------------------------------------
	...
	public override bool ON()
	{
		...
		if( base.ON(data) )
		{
			...
			{
				if( (productstatic.Type)==(UPDATE_TYPE.Animal) )
				{
	...
	void Register( tagProductStatic productstatic )
	{
		...
		{
			uiDepartment department = app.ViewBusiness.FindFromType(DEPARTMENT.사육부);
			if( (department)!=(null) )
			{
				if( department.Make().Get()==(productstatic as tagAppsItem) )
				{
					icon.Hold( app.Language.Get(TEXT.현재_사육중인_가축입니다) );
				}
			}
		}
	...
	// 사업체를 건설하기 위한 함수
	//-------------------------------------------------------------------------------------------------------------------------------
	void funcBreed( object wParam=(null), object lParam=(null) )
	{
		//---------------------------------------------------------------------------------------------------------------------------
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		...
		// -
		//---------------------------------------------------------------------------------------------------------------------------
		Indigator( app.Language.Get(TEXT.가축을_입양중입니다) );
//		LoveSignalHash.Breed( app.Business.Get(), (wParam as string), (lParam as tagProductStatic) );
	...
	public override bool ONCONTROL( objControl control, CONTROL_ACTION Action, tagFocus focus )
	{
		...
		foreach( uiIcon icon in GetList() )
		{
			...
			{
				app.Confirm.ON( Func.Script( app.Language.Get(TEXT.__를_사육하시겠습니까), app.Language.Get(app.ProductStatic.GetRawName(icon.Get())) ), (CONFIRM.YESNO), (funcBreed), app.Department.GetID(), icon.Get() );
				return true;
			}
	...
	public override void Loader()
	{
		...
		if( (subject)!=(null) )
		{
			(subject.text)	= app.Language.Get(TEXT.가축_사육);
		}
